...
/// <summary>
/// Registration for <see cref="IConnectionStatusMonitor"/>.
/// </summary>
public static class ServiceCollectionExtensions
{
    /// <summary>
    /// Registers a scoped <see cref="IConnectionStatusMonitor"/>. Call
    /// <see cref="IConnectionStatusMonitor.StartAsync(CancellationToken)"/> once the app is
    ...
    /// <see cref="ConnectionStatusOptions.PingUrl"/> to get the stronger signal.
    /// </param>
    /// <returns>The same collection, for chaining.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="services"/> is null.</exception>
    /// <exception cref="ArgumentException">
    /// The configured intervals cannot work together; see
    /// <see cref="ConnectionStatusOptions.Validate"/>.
    /// </exception>
    public static IServiceCollection AddBlazorConnectionStatus(
        this IServiceCollection services,
        Action<ConnectionStatusOptions>? configure = null)
    {
        ...
        services.AddScoped<IConnectionStatusMonitor>(provider =>
        {
            var jsRuntime = provider.GetRequiredService<IJSRuntime>();
            return new ConnectionStatusMonitor(new ConnectionStatusInterop(jsRuntime), options);
        });
